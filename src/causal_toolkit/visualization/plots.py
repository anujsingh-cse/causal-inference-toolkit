...
class ForestPlot:
    ...
    def plot(
        ...
        **kwargs,
        ...
        if any("is_overall" in e for e in estimates):
            ...
            self._plot_diamond(
                ax,
                point_estimates[overall_idx],
                ci_lower[overall_idx],
                ci_upper[overall_idx],
                y_pos[overall_idx],
            )
    ...
    def plot_interactive(
        self, estimates: list[dict], xlabel: str = "Effect Size", title: str = "Forest Plot"
    ) -> go.Figure:
...
class LovePlot:
    ...
    def plot(
        ...
        **kwargs,
        ...
        # Threshold lines
        ax.axvline(
            threshold, color="red", linestyle="--", alpha=0.5, label=f"Threshold ({threshold})"
        )
...
class UpliftPlot:
    ...
    def plot_qini(
        ...
        **kwargs,
        ...
        ax.plot(
            x_vals,
            np.cumsum(y_vals) / np.sum(np.abs(y_vals))
            if np.sum(np.abs(y_vals)) > 0
        ...
        ax.plot(
            x_vals,
            np.cumsum(random_y) / np.sum(np.abs(random_y))
            if np.sum(np.abs(random_y)) > 0
            else np.zeros_like(random_y),
            "r--",
            linewidth=1,
            label="Random",
        )
    ...
    def plot_interactive_qini(
        self, uplift: np.ndarray, treatment: np.ndarray, outcome: np.ndarray, n_bins: int = 10
    ) -> go.Figure:
    ...
    def plot_distributions(
        ...
        **kwargs,
        ...
        axes[0].axvline(
            ...
            linestyle="--",
            linewidth=2,
            label=f"Mean: {np.mean(y0_samples):.2f}",
        )
        ...
        axes[1].axvline(
            ...
            linestyle="--",
            linewidth=2,
            label=f"Mean: {np.mean(y1_samples):.2f}",
        )
